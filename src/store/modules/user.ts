...
import { toTree, mapTree, isExternalLink } from 'eui-admin-kit';
import type { ProMenuItem } from 'eui-admin-kit/es/ProLayout/types';
...
export const useUserStore = defineStore('user', {
  ...
  actions: {
     ...
     * 更新菜单数据
     */
    setMenus(menus: ProMenuItem[] | null) {
      this.menus = menus;
    },
    /**
     * 更新菜单的徽章
...
function formatMenus(data: Menu[], childField = 'children') {
  ...
  const menus = mapTree<Menu, ProMenuItem>(
    data,
    (item) => {
      const meta: ProMenuItem['meta'] =
        typeof item.meta === 'string'
          ? JSON.parse(item.meta || '{}')
          : item.meta;
      const menu: ProMenuItem = {
        path: item.path,
