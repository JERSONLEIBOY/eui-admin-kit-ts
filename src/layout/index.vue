...
<script lang="ts" setup>
  ...
  import {
    ...
    checkFullscreen,
    EuiMessage
  } from 'eui-admin-kit/es';
  import type {
    MenuI18n,
    TabItemEventOption,
    BodySizeChangeOption
  } from 'eui-admin-kit/es/ProLayout/types';
  import type { DropdownItem } from 'eui-admin-kit/es/Dropdown/types';
  import {
    MenuFoldOutlined,
    MenuUnfoldOutlined,
    ReloadOutlined,
  ...
  /** 页签点击事件 */
  const onTabClick = (option: TabItemEventOption) => {
  ...
  /** 内容区尺寸改变事件 */
  const onBodySizeChange = ({ width }: BodySizeChangeOption) => {
  ...
  const toggleFullscreen = () => {
    ...
    try {
      ...
      console.error(e);
      EuiMessage.error('您的浏览器不支持全屏模式');
    }
  };

  /** 页签右键菜单点击事件 */
  const onTabContextMenu = (option: TabItemEventOption) => {
  ...
  /** 页签栏右侧下拉菜单点击事件 */
  const onTabDropdownMenu = (command: any, active: string) => {
    if (command === 'reload') {
      reloadPageTab();
