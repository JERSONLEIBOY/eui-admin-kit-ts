<template>
  <div>
    ...
    >
      <template #toolbar>
        <el-button type="primary" class="eui-btn-icon" @click="ok()">
          批量完成
        </el-button>
      ...
      <template #status="{ row }">
        <eui-text v-if="row.status === 0" type="warning">未完成</eui-text>
        <eui-text v-else-if="row.status === 1" type="info">已完成</eui-text>
      </template>
      <template #action="{ row }">
        <el-link type="primary" :underline="false" @click="ok(row)">
          完成
        </el-link>
