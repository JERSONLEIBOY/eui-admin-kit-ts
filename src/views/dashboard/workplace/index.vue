<template>
  <eui-page class="workplace-page">
    ...
    <el-row :gutter="16" ref="wrapRef">
      ...
      >
        <component
          :is="item.name"
          :title="item.title"
          @command="(command: Command) => onCommand(command, index)"
        />
      </el-col>
    </el-row>
    <eui-card :body-style="{ padding: 0 }">
    ...
    <!-- 添加卡片弹窗 -->
    <eui-modal :width="680" v-model="visible" title="未添加的卡片">
...
<script lang="ts" setup>
  ...
  import { EuiMessage } from 'eui-admin-kit';
  ...
  /** 编辑卡片 */
  const onCommand = (command: Command, index: number) => {
    switch (command) {
      case 'edit': // 编辑
        EuiMessage.info({ message: '点击了编辑', plain: true });
        break;
      case 'remove': // 删除
...
<style lang="scss" scoped>
  .workplace-page {
    :deep(.eui-card-header) {
      cursor: move;
    }

    :deep(.el-col) {
      &.sortable-chosen > .eui-card {
