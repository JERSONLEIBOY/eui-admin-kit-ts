<template>
  ...
  <EleTabs
    ...
    type="border-card"
    :modelValue="item.children?.length ? item.children[0].prop : void 0"
    v-bind="item.props || {}"
    :items="
      item.children
        ? item.children.map((c) => ({
  ...
  <EleTable
    v-else-if="
      item.type === 'div' &&
      item.props?.is &&
      ['eui-table', 'EleTable'].includes(item.props.is)
    "
...
<script lang="ts" setup>
  ...
  import { ElCollapse, ElCollapseItem } from 'element-plus/es';
  ...
  // 用于 div 的 is
  defineOptions({ components: { ElCollapse, ElCollapseItem } });
