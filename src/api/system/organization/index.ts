import request from '@/utils/request';
import type { ApiResult, PageResult } from '@/api';
import type { Organization, OrganizationParam } from './model';

/**
 * 分页查询机构
 */
export async function pageOrganizations(params: OrganizationParam) {
  const res = await request.get<ApiResult<PageResult<Organization>>>(
    '/organizations/page',
    { params }
  );
  if (res.data.code === 200) {
    return res.data.data;
  }
  return Promise.reject(new Error(res.data.message));
}

/**
 * 查询机构列表
 */
export async function listOrganizations(params?: OrganizationParam) {
  const res = await request.get<ApiResult<Organization[]>>('/organizations', {
    params
  });
  if (res.data.code === 200 && res.data.data) {
    return res.data.data;
  }
  return Promise.reject(new Error(res.data.message));
}

/**
 * 添加机构
 */
export async function addOrganization(data: Organization) {
  const res = await request.post<ApiResult<unknown>>('/organizations', data);
  if (res.data.code === 200) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
}

/**
 * 修改机构
 */
export async function updateOrganization(data: Organization, id?: string) {
  const res = await request.put<ApiResult<unknown>>(
    '/organizations/' + id,
    data
  );
  if (res.data.code === 200) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
}

/**
 * 删除机构
 */
export async function removeOrganization(id?: string) {
  const res = await request.delete<ApiResult<unknown>>('/organizations/' + id);
  if (res.data.code === 200) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
}
